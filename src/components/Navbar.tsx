    ...
    <nav className={cn(
      ...
      <div className={cn(
        ...
        <div className="hidden lg:flex items-center gap-8 xl:gap-12 flex-1 justify-center px-8">
          {navItems.map((item) => (
            <Link
              key={item.label}
              href={item.href}
              className="text-[10px] font-bold text-black/50 hover:text-primary transition-all uppercase tracking-[0.3em] whitespace-nowrap relative after:content-[''] after:absolute after:-bottom-1 after:left-0 after:w-0 after:h-[1.5px] after:bg-primary after:transition-all hover:after:w-full"
            >
              {item.label}
            </Link>
          ))}
        ...
        <div className="flex items-center gap-4 md:gap-8 shrink-0">
          <Link 
            href="/resume" 
            className="hidden md:flex items-center gap-2.5 text-[10px] font-bold uppercase tracking-[0.2em] text-black/80 hover:text-primary transition-colors group"
          >
            <FileText className="w-4 h-4 text-black/40 group-hover:text-primary transition-colors" />
            RESUME
          </Link>
          <Button asChild className="rounded-full bg-primary text-white hover:bg-primary/90 h-10 px-8 text-[10px] font-black uppercase tracking-[0.3em] shadow-[0_0_20px_rgba(var(--primary),0.2)] hover:shadow-primary/30 transition-all duration-300">
