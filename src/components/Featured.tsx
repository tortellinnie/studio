            ...
            return (
              <Link href={`/impact/${item.id}`} key={item.id} className="group">
                <Card className="glass-card border-white/5 overflow-hidden rounded-[2rem] h-full flex flex-col premium-gradient-border">
                  ...
                  <CardContent className="p-8 lg:p-10 space-y-6 flex-grow relative">
                    <div className="flex items-center justify-between">
                      <Badge className="bg-primary/20 text-primary border-primary/20 backdrop-blur-xl font-black uppercase text-[9px] tracking-widest h-8 px-4">
                        <item.icon className="w-3 h-3 mr-2" />
                        {item.type}
                      </Badge>
                      <div className="flex items-center gap-2 text-muted-foreground font-bold">
                        <Calendar className="w-3.5 h-3.5 text-primary" />
                        <span className="text-xs uppercase tracking-tight">{item.date}</span>
                      </div>
                    </div>
                    <div className="space-y-3">
                      <div className="flex items-start justify-between gap-4">
                        <h3 className="text-2xl lg:text-3xl font-black text-white group-hover:text-primary transition-colors leading-tight tracking-tighter">
                          {item.title}
                        </h3>
                      ...
                      <div className="flex items-center gap-2 text-muted-foreground">
                        ...
                        <span className="text-xs font-bold uppercase tracking-[0.15em]">{item.location}</span>
                      </div>
                    </div>
                    <p className="text-base lg:text-lg text-muted-foreground leading-relaxed italic font-medium opacity-80 group-hover:opacity-100 transition-opacity line-clamp-2">
                      "{item.description}"
                    </p>
                    <div className="pt-4">
                      <div className="inline-flex items-center gap-2 text-[10px] font-black uppercase tracking-[0.3em] text-primary border-b-2 border-primary/0 group-hover:border-primary transition-all pb-1">
                        Read Full Story
                      </div>
                    </div>
                  </CardContent>
